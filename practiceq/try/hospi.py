...
my_image = ImageTk.PhotoImage(Image.open("hosp.jpg"))
my_label = Label(image=my_image)
image1 = Label(root, image=my_image).place(x=0, y=0, relwidth=1, relheight=1)

conn = sqlite3.connect("record.db")  # creating table
c = conn.cursor()
'''
...
def exit():  # to exit the window
    iexit = messagebox.askyesno("pharmecy management", "confirm if you want to quit", parent=top)
    if iexit > 0:
        root.destroy()  # it will exit the window
        return


def clear():  # to clear the text
    comember.set("")
    txtrefenceno.delete(0, END)
    txtdose.delete(0, END)
    txtlot.delete(0, END)
...
def fetch_data():  # to fetch data from table
    conn = sqlite3.connect('record.db')
...
def get_cursor(event=""):  # creating cursor on table
...
def prescreptiondata():  # showing the data
    ...
    txtprescription.insert(END,
                           "xx----------------------------------------------------------------------------------xx" + "\n")


def update():  # to update data
    conn = sqlite3.connect('record.db')
    c = conn.cursor()
    # update a record
...
def delete():  # to delete data
    conn = sqlite3.connect('record.db')
    c = conn.cursor()

    c.execute("DELETE from record3 WHERE patient_Id =" + pid.get())

    conn.commit()
    conn.close()
    fetch_data()
    messagebox.showinfo("delete", "record has been deleted", parent=top)


def hospital():  # creating new window name hospital
    global my_hospital
    global top
    top = Toplevel()
    top.iconbitmap("lg.ico")
    ...
    b1 = Button(top, image=myimg, borderwidth=0)
    b1.place(x=110, y=10)
    ...
    # creating global variable to use it out of function
    global comember
    global txtrefenceno
    global txtdose
    ...
    btndelete = Button(buttonframe, command=delete, text="delete", font=("arial", 12, "bold"), width=20, height=2,
